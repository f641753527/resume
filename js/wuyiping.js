...
function F_animateChengdu() {
    ...
    function production(){
	    ...
	    var nameTxt = "姓名  ：   范浩润";
	    var phoneTxt = "电      话  ：  130****2230";
	    var emailTxt = "邮      箱  ：  fan_pj@formssi.com";
	    var eduTxt = "湖北工业大学    计算机科学与技术    2013.9---2017.7";
	    /*bamboo.innerHTML = str;*/
	    var timerPJ = setInterval(function(){
	    	name.innerHTML += nameTxt.substr(0,1);
	    	phone.innerHTML += phoneTxt.substr(0,1);
	    	email.innerHTML += emailTxt.substr(0,1);
	   		edu.innerHTML += eduTxt.substr(0,1)
	   
	    	nameTxt = nameTxt.substr(1,nameTxt.length-1);
	    	phoneTxt = phoneTxt.substr(1,phoneTxt.length-1);
	    	emailTxt = emailTxt.substr(1,emailTxt.length-1);
	    	eduTxt = eduTxt.substr(1,eduTxt.length-1);
	    	if(eduTxt.length==0){
